...
describe("Wait time / Backoff", () => {
	...
	it("should backoff linearly when configured directly", async () => {
		const task = createTask(Array(4))
		expect.assertions(4)
		
		await sisyphus().triesTo(task)
			.backingOff().linearly(500, 100)
			.now()

		expect(setTimeout).toBeCalledTimes(3)
		expect(setTimeout).toHaveBeenNthCalledWith(1, expect.anything(), 500)
		expect(setTimeout).toHaveBeenNthCalledWith(2, expect.anything(), 600)
		expect(setTimeout).toHaveBeenNthCalledWith(3, expect.anything(), 700)
	})

	it("should backoff linearly when configured fluently", async () => {
		const task = createTask(Array(4))
		expect.assertions(4)
		
		await sisyphus().triesTo(task)
			.backingOff().linearly()
				.startingAt(500)        // eslint-disable-line indent
				.incrementingBy(100)    // eslint-disable-line indent
			.now()
	...
	it("should backoff exponentially when configured directly", async () => {
		const task = createTask(Array(7))
		expect.assertions(7)
		
		await sisyphus().triesTo(task)
			.backingOff().exponentially(100, 2)
			.now()

		expect(setTimeout).toBeCalledTimes(6)
		expect(setTimeout).toHaveBeenNthCalledWith(1, expect.anything(), 100)
		expect(setTimeout).toHaveBeenNthCalledWith(2, expect.anything(), 200)
		expect(setTimeout).toHaveBeenNthCalledWith(3, expect.anything(), 400)
		expect(setTimeout).toHaveBeenNthCalledWith(4, expect.anything(), 800)
		expect(setTimeout).toHaveBeenNthCalledWith(5, expect.anything(), 1600)
		expect(setTimeout).toHaveBeenNthCalledWith(6, expect.anything(), 3200)
	})

	it("should backoff exponentially when configured fluently", async () => {
		const task = createTask(Array(7))
		expect.assertions(7)
		
		await sisyphus().triesTo(task)
			.backingOff().exponentially()
				.startingAt(100)        // eslint-disable-line indent
				.withFactor(2)          // eslint-disable-line indent
			.now()
